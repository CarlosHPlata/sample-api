...
readonly CI_WORKFLOW=on_pr.yml   # the workflow in .github/workflows/ that must be green
...
CI ($CI_WORKFLOW) passed for this commit, and that npm test passes.
...
preflight() {
  ...
  [[ -d node_modules ]] || die "node_modules is missing; run 'npm ci' first"
...
# npm test and node_modules are relative to the repo root, wherever this was run from.
